...
│   ├── common/openf1-mappers.ts   # toRaceControlMessage(), toRaceOvertake(), toTeamRadioClip(), toTrackLocation() — split out from mappers.ts once the OpenF1-sourced DTOs pushed it over the 150-line budget
...
│   │   │   ├── track_locations.ts   # OpenF1 downsampled car x/y per race per driver (bigserial — high row count); one driver FK
│   │   │   ├── lap_times.ts
...
│       │   ├── race-events.service.ts    # getRaceControlMessages(), getOvertakes(), getTeamRadio(), getTrackReplay(), raceExists()
│       │   ├── race-events.controller.ts
│       │   └── race-events.module.ts     # GET /:id/race-control, /:id/overtakes, /:id/team-radio, /:id/track-replay (mounted at /api/races)
│       ├── drivers/
...
│   │       ├── mappers.test.ts
│   │       ├── openf1-mappers.test.ts # toRaceControlMessage/toRaceOvertake/toTeamRadioClip/toTrackLocation
│   │       ├── standings.test.ts
...
│   │   ├── race-events/race-events.test.ts # race-control message + overtakes + team-radio + track-replay lists, ordering, 404
...
│   │       ├── TeamRadioList.astro    # Team radio clip list with custom-built playback controls (native <audio controls> didn't fit the dark UI)
│   │       ├── TrackReplay.astro      # Plain-SVG post-race car-position replay; downsampled track_locations animated via requestAnimationFrame, JSON payload embedded like DriverRadarCompare.astro
...
│   │                              #   RaceControlMessage, RaceOvertake, TeamRadioClip, TrackLocation
│   ├── styles/
...
│   │   ├── ingest_track_replay.py      # OpenF1 location → track_locations; per-driver fetch + _downsample() (~1 point/2s, ~5-7x fewer rows than raw 3.7Hz)
...
│       ├── openf1_client.py       # get_session_key(), fetch_race_control(), fetch_overtakes(), fetch_team_radio(), fetch_location() — thin OpenF1 REST client (free/historical tier only)
...
│   ├── backfill_track_replay.py   # Backfill OpenF1 downsampled track locations for a year range (2023+ only — no OpenF1 coverage before)
...
│   ├── test_ingest_track_replay.py # _downsample() interval logic + run() per-driver fetch/upsert — via fake_db
│   ├── test_openf1_client.py       # get_session_key (date match incl. 1-day drift, pre-2023 no-coverage), fetch_race_control, fetch_overtakes, fetch_team_radio, fetch_location
...
| `ingest_track_replay` | `--year --round` | OpenF1 downsampled car x/y positions — same 2023+/completed/live-window gating; one OpenF1 call per driver, ~1 point/2s kept (vs raw 3.7Hz) |
