"""Backfills the ingest_overtakes job (OpenF1) for every completed race in a year range.
Usage: python scripts/backfill_overtakes.py 2023 2026"""
import random
import sys
import time
...
from src.jobs.ingest_overtakes import run as ingest_overtakes

# Each race makes 2 OpenF1 requests (sessions lookup + overtakes fetch). OpenF1's free
...
def safe(year: int, round_num: int) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ingest_overtakes(year, round_num)
            return True
...
if __name__ == "__main__":
    ...
    print(f"Backfilling overtakes for {len(races)} completed races ({start}-{end})")
