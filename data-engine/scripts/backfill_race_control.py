"""Backfills the ingest_race_control job (OpenF1) for every completed race in a year range.
Usage: python scripts/backfill_race_control.py 2020 2026"""
import random
import sys
import time
...
from src.jobs.ingest_race_control import run as ingest_race_control

# Each race makes 2 OpenF1 requests (sessions lookup + race_control fetch). OpenF1's free
...
def safe(year: int, round_num: int) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ingest_race_control(year, round_num)
            return True
...
if __name__ == "__main__":
    ...
    print(f"Backfilling race control messages for {len(races)} completed races ({start}-{end})")
