"""Backfills the ingest_track_replay job (OpenF1) for every completed race in a year range.
Usage: python scripts/backfill_track_replay.py 2023 2026"""
import random
import sys
import time
...
from src.jobs.ingest_track_replay import run as ingest_track_replay

# Each race already makes ~20+ paced OpenF1 requests internally (one per driver, plus the
# session lookup), so the extra gap between races only needs to cover the boundary.
SLEEP_BETWEEN_RACES_S = 2.0
...
def safe(year: int, round_num: int) -> bool:
    ...
    for attempt in range(max_retries):
        try:
            ingest_track_replay(year, round_num)
            return True
...
if __name__ == "__main__":
    ...
    print(f"Backfilling track replay data for {len(races)} completed races ({start}-{end})")
