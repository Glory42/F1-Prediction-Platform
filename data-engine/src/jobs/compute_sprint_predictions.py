...
def run(race_id: int) -> None:
    print(f"[compute_sprint_predictions] race_id={race_id}")

    conn = get_conn()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT driver_id, raw_weighted_score FROM driver_sprint_features "
        ...
        if not feature_rows:
            raise ValueError(f"No sprint feature rows for race {race_id} — run compute_sprint_features first")
        ...
        conn.commit()
        print(
            f"  Sprint predicted winner: driver_id={predicted_winner_id} "
