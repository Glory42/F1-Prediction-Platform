---
name: codebase-audit
description: Full health audit of the F1 Prediction codebase — critical-constraint violations (TCP drivers in api/, inline schema, client-side data fetching), ETL idempotency gaps, schema/migration drift, missing "any" types, and stale race-status flow handling. Use when the user says "audit the codebase", "health check", "/codebase-audit", or before a season backfill or after a long feature sprint.
---

# Codebase Audit — F1 Prediction

A structured health check across `api/`, `web/`, `data-engine/`, and `db/`. Run before a big backfill, before a release, or when the codebase feels like it's accumulated debt. Reports issues grouped by severity; does not auto-fix anything unless the user asks.
...
- **TCP driver in `api/`**: grep `api/` for `from 'pg'`, `require('pg')`, `postgres(` (the `postgres` npm package), or any import that isn't `@neondatabase/serverless`. Cloudflare Workers have no TCP support — this breaks at runtime, not compile time.
  ```bash
  grep -rn "from 'pg'\|from \"pg\"\|require('pg')\|from 'postgres'" api/src
  ```
- **Inline schema definitions**: grep route/controller/service files in `api/src/modules/` for `pgTable(` — schema must live only in `api/src/db/schema/`.
- **Client-side data fetching**: grep `web/src` for `client:load`, `client:idle`, `client:visible` directives combined with a `.tsx` island that calls `fetch(`. CLAUDE.md mandates all data fetching happens in Astro frontmatter, never in `client:*` islands.
- **API write endpoints**: grep `api/src/modules/*/controller.ts` (or `*.controller.ts`) for `app.post(`, `app.put(`, `app.delete(`, `app.patch(`. The API is documented as read-only — Python writes directly to Neon. Any write route is a constraint violation unless the user has explicitly changed this rule.
...
```bash
grep -rn "'scheduled'\|'qualifying_done'\|'sprint_qualifying_done'\|'sprint_done'\|'completed'" api/src data-engine/src --include="*.ts" --include="*.py"
```

### 4. `any` types (CLAUDE.md Code Style: "No `any` types")
```bash
grep -rn ": any\b\|<any>\|as any" api/src web/src --include="*.ts" --include="*.tsx"
```
Group by file. Known existing spots as of this audit's baseline: `api/src/main.ts` (env passthrough), `api/src/common/mappers.ts` (globalThis process access), `api/src/modules/races/races.service.ts` (era/dominance aggregation types). Flag any *new* `any` beyond a previously-reported baseline as the priority; long-standing ones can be listed as existing debt.
...
```bash
find api/src web/src data-engine/src -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.astro" -o -name "*.py" \) | xargs wc -l | sort -rn | head -15
```
...
Check `api/src/main.ts` — `PROD_ORIGINS` / `DEV_ORIGINS` gating should stay behind `c.env.ENVIRONMENT === 'production'`. Flag if a new hardcoded origin was added without going through this gate, or if `DEV_ORIGINS` leaked into the production branch.

### 8. Unresolved TODOs and FIXMEs
```bash
grep -rn "TODO\|FIXME\|HACK\|XXX" api/src web/src data-engine/src --include="*.ts" --include="*.tsx" --include="*.py"
```
