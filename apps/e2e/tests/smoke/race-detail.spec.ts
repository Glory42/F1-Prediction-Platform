...
test('renders the overtakes tab with driver codes resolved', async ({ page }) => {
  await page.goto('/races/1');

  await page.getByRole('tab', { name: 'Overtakes' }).click();
  const panel = page.locator('#panel-overtakes');
  await expect(panel.getByText('VER')).toBeVisible();
  await expect(panel.getByText('LEC')).toBeVisible();
